...
const float alpha = 50.0;//const float alpha = 2500.0;
const float k = 1.0;
const float k1 = 0.1;
const float b = 0.06;
const float gA = 15.f;//15.0;
const float gI = 24.f;//24.0;
const float gH = 0.1f;//0.1;
const float f = 0.3;
const float g = 0.01;
const float D = 2.5f;//2.5;
const float mu = 1.0;
const float mat = 0.001;
float A = species[0];
float I = species[1];
float Hi = species[2];
float P = species[3];
...
rates[2] = b*I/(1.f+k*I) - gH*A*Hi/(1.f+g*Hi) + D*(He-Hi);
//rates[2] = b*I/(1.f+k*I) - gH*A*Hi/(1.f+g*A) + D*(He-Hi);
rates[4] = mat*P;
"""

cl_sig_rate = """
rates[0] = D*(Hi-He)*volume;
"""
...
def update(cells):
    ...
    for (i,cell) in cells.items():
        cell.color = [0.2+cell.species[0]/2300.0, 0.2+cell.species[1]/8000.0, 0.2+cell.species[2]/0.35]

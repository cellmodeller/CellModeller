...
def setup(sim):
    ...
    biophys = CLBacterium(sim, max_substeps=8, max_cells=max_cells, max_contacts=32, max_sqs=192**2, jitter_z=False, reg_param=0.04, gamma=25)
...
def update(cells):
    for (id, cell) in cells.iteritems():
        cell.color = [cell.strain*10,0,0] #cell_colors[cell.cellType]
        if cell.volume > cell.targetVol:
            cell.asymm = [1,1]
            cell.divideFlag = True

        dl = cell.volume - cell.prevlen
        cell.strain = dl/cell.volume
        cell.prevlen = cell.volume

def divide(parent, d1, d2):
    d1.targetVol = rodlen + random.uniform(0.0,rodlen/7.0)
    d2.targetVol = rodlen + random.uniform(0.0,rodlen/7.0)
    pvol = parent.volume
    pplen = parent.prevlen
    d1.volume = pvol*0.5
    d2.volume = pvol*0.5
    d1.prevlen = pplen*0.5
    d2.prevlen = pplen*0.5
    d1.strain = parent.strain
    d2.strain = parent.strain

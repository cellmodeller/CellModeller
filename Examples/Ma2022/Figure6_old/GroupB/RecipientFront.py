...
def setup(sim):
    ...
    ################ Place recipients at frontier: in total 305 cells; 152 recipients, 153 donors. almost 1:1 ratio.
    ...
    for m in range(-19,20):
        for n in range(-19,20):
            ...
            if R < 55:
                points = (m,n)
                coord.append(points)
    points_rd = random.sample(coord,218) #### randomly pick 152 coordinates
    ...
    for element in points_rd:
        x = element[0]
        y = element[1]
        sim.addCell(cellType = 1, pos = (x*dist,y*dist,0) , dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate donors

...
def setup(sim):
    ...
    ################ Place producer at frontier: in total 305 cells; 152 recipients, 153 donors. almost 1:1 ratio.
    ...
    for m in range(-19,20):
        for n in range(-19,20):
            ...
            if R < 55:
                points = (m,n)
                coord.append(points)
    points_rd = random.sample(coord,219) #### randomly pick 219 coordinates
    ...
    for element in points_rd:
        x = element[0]
        y = element[1]
        sim.addCell(cellType = 0, pos = (x*dist,y*dist,0) , dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate recipients
    ...
    for i in range(-19,20):
        for j in range(-19,20):
            ...
            if R >=55 and R < radius:
                sim.addCell(cellType=1, pos=(i*dist,j*dist,0), dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate donors at front

    for element_donor in donors_within:
        e = element_donor[0]
        f = element_donor[1]
        sim.addCell(cellType=1, pos=(e*dist,f*dist,0), dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate donors behind.

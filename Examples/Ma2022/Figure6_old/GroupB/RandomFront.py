...
def setup(sim):
    ...
    ################ Place random donors and consumers at frontier: in total 305 cells; 152 recipients, 153 donors. almost 1:1 ratio.
    ...
    for m in range(-19,20):
        for n in range(-19,20):
            ...
            if R < radius:
                points = (m,n)
                coord.append(points)
    points_rd = random.sample(coord,219) #### randomly pick 153 coordinates for donors.
    recipients_within = list(set(coord) - set(points_rd)) ###### get 152 complementary coordinates for recipients behind the frontier.

    for element in points_rd:
        x = element[0]
        y = element[1]
        sim.addCell(cellType = 1, pos = (x*dist,y*dist,0) , dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate donors randomly.

    for element_donor in recipients_within:
        e = element_donor[0]
        f = element_donor[1]
        sim.addCell(cellType=0, pos=(e*dist,f*dist,0), dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate recipients randomly.
...
def init(cell):
    ...
    #    cell.growthRate = 1.1
    if cell.cellType == 0:
        cell.color = (1.0, 0.1, 0.1)#red consumer, recipients
    elif cell.cellType == 1:
        cell.color = (0.32, 1.0, 0.96) #cyan producer donors
            ...
            for index in cell.neighbours:#loop through all contacts
                if cells[index].cellType == 1: #if donor or transconjugant is in contact
                    if random.random() < 0.0005: # constant probability per unit time
                        sim.HGTevents += 1
                        cell.cellType = 2 #become transconjugant yellow
                        cell.growthRate = v_max * cell.species[0] / (Km + cell.species[0])
                        cell.color = (0.98, 1.0, 0.15)
        if sim.stepNum > 400: #antibiotic added at time 400
            if cell.cellType == 0: #if a cell is an recipient,
                cell.growthRate = 0.0
            elif cell.cellType == 1: #if a cell is a transconjugant -- producer:
                cell.growthRate = 1
            else:
                cell.growthRate = v_max * cell.species[0] / (Km + cell.species[0])

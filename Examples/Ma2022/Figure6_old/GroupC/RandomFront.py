...
def setup(sim):
    ...
    for m in range(-19,20):
        for n in range(-19,20):
            ...
            if R < radius:
                points = (m,n)
                coord.append(points)
    points_rd = random.sample(coord,218) #### randomly pick 218 coordinates for donors.
    ...
    for element in points_rd:
        x = element[0]
        y = element[1]
        sim.addCell(cellType = 0, pos = (x*dist,y*dist,0) , dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate donors randomly.

    for element_donor in recipients_within:
        e = element_donor[0]
        f = element_donor[1]
        sim.addCell(cellType=1, pos=(e*dist,f*dist,0), dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate recipients randomly.

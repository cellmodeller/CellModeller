...
def setup(sim):
    ...
    ################ Place recipients at frontier: in total 437 cells; 218 donors, 219 recipients. almost 1:1 ratio.
    ...
    for m in range(-19,20):
        for n in range(-19,20):
            ...
            if R < 55:
                points = (m,n)
                coord.append(points)
    points_rd = random.sample(coord,218) #### randomly pick 153 coordinates for donors
    recipients_within = list(set(coord) - set(points_rd)) ###### get complementary coordinates for recipients behind the frontier.

    for element in points_rd:
        x = element[0]
        y = element[1]
        sim.addCell(cellType = 0, pos = (x*dist,y*dist,0) , dir=(random.uniform(-1,1),random.uniform(-1,1),0), length=delta,rad=0.4) ## inoculate donors

    ### To inoculate recipients at frontier.
